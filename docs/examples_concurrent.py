...
def concurrent_search_example():
    ...
    with ThreadPoolExecutor(max_workers=max_streams) as executor:
        ...
        google_futures = {
            executor.submit(search_google, query): query for query in queries
        }

        # Запускаем поиск в Yandex
        yandex_futures = {
            executor.submit(search_yandex, query): query for query in queries
        }
        ...
        for future in as_completed(google_futures):
            ...
            print(
                f"Google: {result['query']} - {result.get('total_results', 'error')} результатов"
            )
        ...
        for future in as_completed(yandex_futures):
            ...
            results.append(result)
            print(
                f"Yandex: {result['query']} - {result.get('total_results', 'error')} результатов"
            )
...
def batch_processing_example():
    ...
    for batch_num, batch in enumerate(
        batches[:3]
    ):  # Обрабатываем только первые 3 батча
        print(f"\n🔄 Обработка батча {batch_num + 1}/{len(batches)}")

        batch_results = []
        with ThreadPoolExecutor(
            max_workers=limits["max_concurrent_streams"]
        ) as executor:
            futures = {
                executor.submit(google.search, query): query
                for query in batch[
                    :10
                ]  # Обрабатываем только первые 10 запросов для примера
            }
