        ...
        try:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    print(
                        f"✅ Запрос {i+1} ({queries[i]}): {result.total_results} результатов"
                    )
...
async def mixed_google_yandex_example():
    ...
    async with (
        AsyncGoogleClient(user_id=123, api_key="your_google_key") as google,
...
async def retry_configuration_example():
    ...
    async with AsyncGoogleClient(
        ...
        try:
            ...
            print(
                f"✅ Поиск с кастомными настройками: {results.total_results} результатов"
            )
        except Exception as e:
            print(f"❌ Ошибка: {e}")
    ...
    async with AsyncGoogleClient(
        user_id=123, api_key="your_google_key", enable_retry=False  # без повторов
    ) as google:
        try:
