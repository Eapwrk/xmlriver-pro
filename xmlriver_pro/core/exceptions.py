...
ERROR_CODES = {
    ...
    102: "Неверный параметр groupby! Возможные значения для Google: 10, 20, 30, 50, 100. Для Яндекс: 10",
    103: "Неверный параметр lr! Может возникнуть при передаче неверного значения параметра",
    104: "Неверный параметр loc! Может возникнуть при передаче неверного значения параметра",
    105: "Неверный параметр country! Может возникнуть при передаче неверного значения параметра",
    106: "Неверный параметр domain! Может возникнуть при передаче неверного значения параметра",
    107: "По Яндексу возможное значение ТОПа: 10! Если в запросе передан параметр groupby и он не равен возможным значениям",
    108: "Не передан параметр zoom или coords для Google карт. Параметры zoom и coords – обязательны для поиска заведений по картам Google",
    110: "Заняты все доступные вам каналы для сбора данных. Попробуйте позже. Вами превышено максимально допустимое количество потоков",
    111: "Нет свободных каналов для сбора данных. Попробуйте позже. По техническим причинам сейчас нет больше свободных каналов",
    ...
    121: "Bad request id. Возвращается при сборе в отложенном режиме если переданный req_id у нас не найден",
    ...
    202: "Запрос ещё не обработан. Ошибка может возникать при работе с Яндекс Search API. Ответ от Яндекса ещё не готов",
    500: "Ошибка сети. Повторите попытку. До 20% ответов с этой ошибкой – нормальное поведение сервиса",
